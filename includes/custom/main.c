...
void my_memcpy(unsigned char* destination, unsigned char* source, int offset, int count) {
  ...
  for (i = 0; i < count; i++) destination[(i + offset) & 255] = source[i] & mask;
}

void my_memset(unsigned char* destination, int fill_value, int count) {
  int i;       // saves a few bytes
  for (i = 0; i < count; i++) destination[i] = fill_value;
}
...
int checkswap(int a, int b) {
  ...
  temp1 = s1 - s2;
  if (temp1 > 0) { // larger is higher
    if ((temp1 -= 5) > 0) {// not overlapping
...
void copynybble(unsigned char num) {
  ...
  for (i = 0; i < 8; i++) destination[i] = source[7 - i];
}
...
int main() {
  ...
  switch (C_function[0] & 0xFC) {
    case 4: { // pfvline xpos ypos endypos function
      ...
      for (i = C_function2; i <= C_function1; ++i) on_off_flip(i, setbyte[C_function3]);
      return;
    }
    case 8: { // pfhline
    ...
    case 32: { // pfscroll
      for (temp3 = C_function2; temp3 < C_function3; temp3++) memscroll(queue + get32bitdff(temp3), C_function1);
      return;
    }
  ...
  for (i = 0; i < maxsprites; i++) myGfxIndex[i] = spritesort[i];
  // loop
  temp3 = maxsprites - 1;
  temp2 = maxsprites - 2;
  while (temp2 >= 0) {
  ...
  for (i = 0; i < maxsprites; i++) RIOT[SpriteGfxIndex + i] = myGfxIndex[i];
  ...
  while (temp4 != 511) {
    ...
    my_memcpy(queue + get32bitdf(3), // queue + (dfhigh(3) << 8) + dflow(3),
    ...
    mask = 0xFF;
    my_memcpy(queue + get32bitdf(1), // queue+(dfhigh(1)<<8)+dflow(1),
