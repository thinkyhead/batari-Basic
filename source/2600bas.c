...
int main(int argc, char *argv[]) {
	...
	while (1) {               // clear out statement cache
		...
		if (code[i + 1] == 'd' && code[i + 2] == 'e' && code[i + 3] == 'f' && code[i + 4] == ' ') {
		 	// found a define
			i += 5;
			for (j = 0; code[i] != ' '; i++) def[defi][j++] = code[i];		// get the define
			def[defi][j] = '\0';

			i += 3;
			for (j = 0; code[i] != '\0'; i++) defr[defi][j++] = code[i];	// get the definition
			defr[defi][j] = '\0';
			removeCR(defr[defi]);
			printf(";.%s.%s.\n", def[defi], defr[defi]);
			defi++;
		...
		else if (strchr(statement[0], '.') != NULL) {
